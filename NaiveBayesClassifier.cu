...
__device__ double calculate_single_word_label_occurence_cuda(int * feature_vectors,int * documents_size, int * documents_indexes, int * docs_labels, int word_index, int number_documents, int label,int num_labels, double* weights)
{
	double result = 0;
...
__global__ void calculate_occurences_kernel(int * d_fv, double* d_fp ,int * documents_size, int * documents_indexes, int * docs_labels, size_t pitch_fp, int num_docs, int num_unique_words, int num_labels, double* weights)
{
	...
	{
		((double*)((char*)d_fp+y*pitch_fp))[x] = calculate_single_word_label_occurence_cuda(d_fv,documents_size,documents_indexes,docs_labels,x,num_docs,y,num_labels,weights);
	}
}
...
__device__ double prob_document_label_cuda(int * feature_vector,double* fp, double*sp, int fv_size, int number_unique_words, int label)
{
...
__device__ int classify_unlabeled_document_cuda(int * unlabeled_feature_vector,double* fp, double* sp, int fv_size, int number_unique_words, int number_labels, double* prob_labels)
{
	double max_prob = N_INF;
	double min_prob = 1;
	int label = -1;
	...
	for(int i = 0;i < number_labels;i++)
	{
		double prob = (double)prob_document_label_cuda(unlabeled_feature_vector, fp, sp, fv_size ,number_unique_words,i);
...
__global__ void classify_unlabeled_docs_kernel(int * unlabeled_feature_vectors, int* doc_sizes, int* docs_indexes, int num_docs ,double* fp, double* sp, int number_unique_words, int number_labels, int* docs_labels, double* prob_labels)
{
	...
	{
		if(prob_labels)
			docs_labels[x] = classify_unlabeled_document_cuda(unlabeled_feature_vectors+docs_indexes[x],fp,sp,doc_sizes[x],number_unique_words,number_labels,&prob_labels[x*number_labels]);
		else 
			docs_labels[x] = classify_unlabeled_document_cuda(unlabeled_feature_vectors+docs_indexes[x],fp,sp,doc_sizes[x],number_unique_words,number_labels,0);
	}
}
